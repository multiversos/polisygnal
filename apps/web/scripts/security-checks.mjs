...
function validateAnalyzeLoadingPanelSource() {
  ...
  assert(walletDetailsSource.includes("Copiar direccion"), "wallet drawer must allow copying public wallet addresses with safe wording");
...
function validateDeepAnalysisJobRules() {
  ...
  assert(walletAnalysisPanelSource.includes("Analizando wallets automaticamente"), "wallet analysis panel should explain automatic incremental batch processing");
  ...
  assert(walletAnalysisPanelSource.includes("Reanudar analisis"), "wallet analysis panel should allow resuming automatic analysis");
  assert(walletAnalysisPanelSource.includes("Confianza de datos"), "wallet analysis panel should separate data confidence from signal strength");
  assert(walletAnalysisPanelSource.includes("Fuerza de senal"), "wallet analysis panel should expose signal strength explicitly");
...
function validateAnalyzerFirstProductSource() {
  ...
  for (const text of ["Resumen", "Wallets", "Copias abiertas", "Auditoria"]) {
    assert(copyTradingDashboard.includes(text), `copy trading tab navigation missing text: ${text}`);
  }
  if (
    copyTradingDashboard.includes('{ id: "closed"') ||
    copyTradingDashboard.includes('activeTab !== "closed"') ||
    copyTradingDashboard.includes('activeTab === "closed"')
  ) {
    assert(
      copyTradingDashboard.includes("Copias cerradas"),
      "copy trading closed-copies tab label should appear once the closed positions panel is integrated",
    );
  }
